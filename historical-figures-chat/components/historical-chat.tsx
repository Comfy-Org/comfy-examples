...
type Message = { id: string; role: "user" | "assistant"; content: string; video?: string };
...
function videoFrom(job: Job) {
  return job.outputs.find((output) => output.type.toLowerCase().includes("video"))?.url ?? job.outputs[0]?.url;
}
...
export function HistoricalChat() {
  ...
  function finishJob(next: Job) {
    ...
    if (next.status === "succeeded") {
      ...
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: "assistant" as const, content: "Response", video }].slice(-MAX_MESSAGES));
      setNotice(video ? "Video response ready." : "The response finished without a video output.");
      return;
    }
  ...
  const latestVideo = useMemo(
    () => [...messages].reverse().find((message) => message.role === "assistant" && message.video)?.video,
    [messages],
  );
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ figureId: figure.id, messages: nextMessages.map(({ role, content }) => ({ role, content })), previousVideoUrl: latestVideo }),
      });
      const next = await response.json() as Job & { error?: string };
